...
ifeq ($(OS),Darwin)
	docker volume create --name=sf5-tasks-api-be-sync
...
generate-ssh-keys: ## Generate ssh keys in the container
	...
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl genrsa -passout pass:sf5-tasks-api -out config/jwt/private.pem -aes256 4096
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl rsa -pubout -passin pass:sf5-tasks-api -in config/jwt/private.pem -out config/jwt/public.pem
